...
template_video() {
    ...
    wallust run "${FRAME_TEMP}" && pkill nwg-wrapper && nwg-wrapper -s "${SCRIPT_RELOGIO}" \
        -c "${CSS_RELOGIO}" -p center -r 1000 &
}
...
if [[ -f "$PATH_WALLPAPER" ]]; then
    ...
    # Imagens e GIFs
        ...
        if awww img "${PATH_WALLPAPER}" \
            ...
            wallust run "${PATH_WALLPAPER}" && pkill nwg-wrapper && nwg-wrapper -s "${SCRIPT_RELOGIO}" \
                -c "${CSS_RELOGIO}" -p center -r 1000 &
